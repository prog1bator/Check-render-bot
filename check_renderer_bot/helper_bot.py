import telebot
from keyboard import (main_keyboard, back_keyboard_check, keyboard_qiwi, checks_and_balances_keyboard, back, tinkoff,
                      screens, sber, admin, rassilka_keyboard, mail)
from telebot.types import InputMediaPhoto
import sqlite3
from PIL import Image, ImageFont, ImageDraw
import time

ADMIN_ID = ''  # Сюда свой айди
token = ''  # Токен бота
...
def start_message(message):
    ...
    if sql.fetchone() is None:
        ...
        bot.send_message(message.chat.id, '🧾Привет! У меня ты сможешь сделать фейк чеки популярных приложений. '
                                          'Надеюсь тебе понравится :)\n⚠️Воспользуйся кнопками для управления ботом\nПо'
                                          ' всем вопросам: @vvhiz', reply_markup=main_keyboard())
        db.commit()
        con.commit()
    else:
        bot.send_message(message.chat.id, '🧾Привет, у меня ты сможешь сделать фейк чеки популярных приложений. '
...
def main(message):
    if message.text == '📝Отрисовка (Чеки, балансы)':
        ...
        bot.send_message(message.chat.id, "Заказать рекламу: @vvhiz", reply_markup=main_keyboard())
        bot.register_next_step_handler(message, main)
        ...
        if message.text:
            if message.text == "Вернуться в Меню🧭":
                ...
                try:
                    ...
                    print(ex)
                    bot.send_message(message.chat.id, 'Что-то пошло не так. Возврат в меню..',
...
def screen(message):
    if message.text == "📦Скрины Авито":
        ...
        bot.send_media_group(message.chat.id,
                             [InputMediaPhoto(a1), InputMediaPhoto(a2), InputMediaPhoto(a3), InputMediaPhoto(a4),
                              InputMediaPhoto(a5)])
        bot.register_next_step_handler(message, screen)
        ...
        bot.send_media_group(message.chat.id,
                             [InputMediaPhoto(y1), InputMediaPhoto(y2), InputMediaPhoto(y3), InputMediaPhoto(y4),
                              InputMediaPhoto(y5)])
        bot.register_next_step_handler(message, screen)
    elif message.text == "Назад":
        bot.send_message(message.chat.id, 'Выберите категорию: ', reply_markup=main_keyboard())
...
def checks_and_balances(message):
    if message.text == "Тинькофф💛":
        ...
        bot.send_message(message.chat.id,
                         "OLX Приватбанк, заполните форму: \n\nСистемное время\nДата и время\nСумма перевода\nОстаток "
...
def sber_menu(message):
    if message.text == "♻️Перевод Сбер♻️":
        ...
        time.sleep(0.2)
        bot.send_message(message.chat.id,
                         "♻️Выбран перевод сбер. Заполните форму:\n\nСистемное время\nИмя получателя\nСумма "
                         "\n\n❗️Разделяйте параметры переносом строки❗️")
        bot.register_next_step_handler(message, sber_transfer)
    elif message.text == "Назад":
        bot.send_message(message.chat.id, 'Выберите категорию: ', reply_markup=main_keyboard())
...
def tinkov(message):
    if message.text == "Тинькофф Перевод":
        ...
        time.sleep(0.1)
        bot.send_message(message.chat.id,
                         "Тинькофф перевод, заполните форму:\n\nСистемное время\nДата и время платежа\nСумма(только "
                         "Рубли и копейки, остальное поставится за вас. Например 1500,00)\nНа карту\n\n❗️Разделяйте "
                         "параметры переносом строки❗️",
                         reply_markup=back())
        ...
        time.sleep(0.1)
        bot.send_message(message.chat.id,
                         "Тинькофф оплата Авито, заполните форму:\n\nСистемное время\nДата и время платежа\nСумма("
                         "только Рубли и копейки, остальное поставится за вас. Например 1500,00)\nПоследние 4 цифры "
                         "вашей карты\nКэшбэк\n\n❗️Разделяйте параметры переносом строки❗️",
                         reply_markup=back())
        ...
        time.sleep(0.1)
        bot.send_message(message.chat.id,
                         "Тинькофф оплата Юла, заполните форму:\n\nСистемное время\nДата и время платежа\nСумма("
                         "только Рубли и копейки, остальное поставится за вас. Например 1500,00)\nПоследние 4 цифры "
                         "вашей карты\nКэшбэк\n\n❗️Разделяйте параметры переносом строки❗️",
                         reply_markup=back())
    ...
    if message.text == "Назад":
        ...
        if message.text:
            if message.text == "Вернуться в Меню🧭":
                ...
                bot.send_message(message.chat.id,
                                 "Тинькофф оплата Юла, заполните форму:\n\nСистемное время\nДата и время "
                                 "платежа\nСумма(только Рубли и копейки, остальное поставится за вас. Например 1500,"
                                 "00)\nПоследние 4 цифры вашей карты\nКэшбэк\n\n❗️Разделяйте параметры переносом "
                                 "строки❗️")
                bot.register_next_step_handler(message, tinkov_youla)
            else:
                try:
...
def qiwi(message):
    if message.text == 'Фейк баланс💰':
        ...
        time.sleep(0.1)
        bot.send_message(message.chat.id,
                         'Введите сумму, номер перевода, дату и время в таком формате:\n151,'
                         '92\n+79XXXXXXXXX\n01.12.2020 в 19:01',
                         reply_markup=back())
...
def qiwi_balance2(message):
    if message.text:
        ...
        bot.send_message(message.chat.id, "Введите текстовый формат")
        bot.register_next_step_handler(message, qiwi_balance2)
